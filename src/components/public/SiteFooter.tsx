// ============================================================
// 站群页脚 — 站名 / 描述 / 友情链接+站群链轮 / R22: 自定义底部文案/版权/备案号
// ============================================================
'use client'
...
export function SiteFooter() {
  ...
  useEffect(() => {
    let alive = true
    fetchFooterLinks(false, site.id).then((d) => { if (alive) setFooterLinks(d) })
    return () => { alive = false }
  }, [site.id])
  ...
  const refreshWheel = () => {
    ...
    setRefreshing(true)
    fetchFooterLinks(true, site.id).then((d) => { if (d) setFooterLinks(d) }).finally(() => setRefreshing(false))
  }
      ...
      <div className="mx-auto w-full max-w-6xl space-y-4 px-4 py-8 sm:px-6">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center text-xs font-black"
              style={{ background: `linear-gradient(135deg, ${v.primary}, ${v.accent})`, color: v.primaryText, borderRadius: v.radius }} aria-hidden>
              {site.name.slice(0, 1)}
            </span>
          ...
          <nav className="flex items-center gap-4 text-xs" aria-label="页脚导航">
            <button type="button" onClick={() => navigate({ view: 'home' })} className="transition-opacity hover:opacity-70" style={{ color: v.textMuted }}>首页</button>
            <button type="button" onClick={() => navigate({ view: 'search', q: '' })} className="transition-opacity hover:opacity-70" style={{ color: v.textMuted }}>搜索</button>
            <button type="button" onClick={() => navigate({ view: 'category' })} className="transition-opacity hover:opacity-70" style={{ color: v.textMuted }}>更多书籍</button>
          </nav>
        </div>
        {site.description && <p className="text-xs leading-relaxed" style={{ color: v.textMuted }}>{site.description}</p>}
        <TagCloud tags={siteKeywordList(site)} />
        <SuggestTagCloud count={12} />
        {footerLinks && (hasFriend || hasWheel) && (
          <nav aria-label="友情链接" className="space-y-1.5 text-xs leading-relaxed"
            style={{ color: v.textMuted, borderTop: `1px dashed ${withAlpha(v.border, 0.7)}`, paddingTop: 12 }}>
            {hasFriend && (
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                <span className="shrink-0 font-medium" style={{ color: v.text }}>友情链接：</span>
                {footerLinks!.friend.map((l) => (
                  <a key={l.id} href={l.url} target="_blank" rel="noopener noreferrer" title={l.name} className="transition-opacity hover:opacity-70">{l.name}</a>
                ))}
              </div>
            )}
            {hasWheel && (
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                <span className="shrink-0 font-medium" style={{ color: v.text }}>站群链轮：</span>
                {footerLinks!.wheel.map((l) => (
                  <a key={l.url} href={l.url} target="_blank" rel="noopener" title={l.text} className="transition-opacity hover:opacity-70">{l.text}</a>
                ))}
                <button type="button" onClick={refreshWheel} disabled={refreshing} aria-label="换一批链轮链接"
        ...
        {/* R22: 自定义底部文案 */}
        {customText && (
          <div className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: v.textMuted, borderTop: `1px dashed ${withAlpha(v.border, 0.7)}`, paddingTop: 12 }}>
            {customText}
          </div>
        )}
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px]" style={{ color: v.textMuted, borderTop: `1px dashed ${withAlpha(v.border, 0.7)}`, paddingTop: 12 }}>
          ...
          {showStats && <span>GEO {site.geoRegion} · {site.geoPlacename}</span>}
        </div>
      </div>
    </footer>
  )
