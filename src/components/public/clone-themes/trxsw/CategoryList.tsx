'use client'
// ============================================================
// clone-trxsw CategoryList — 1:1 精仿天人小说 trxsw.com 分类列表页
// 参考: themes.ts AiraBrowser 反查 DOM (.vlist 章节列表 + .pager 翻页)
//   唐人小说 CMS 通用列表页模板: .breadcrumb + .filter-bar (筛选条) +
//   .list-item (横向 list 列表: cover + info + meta + desc + actions) +
//   .pager 分页 + 侧栏 .hot 同类排行 + .tags 标签云
    ...
    <>
      ...
      <nav className="nav">
        <div className="nav-inner">
          ...
          {navCats.map(c => (
            <a key={c.id} href={`/sort/${c.id}/`} className={'nav-link' + (c.name === label ? ' active' : '')} onClick={(e) => goCat(e, c.id)}>{c.name}</a>
          ))}
