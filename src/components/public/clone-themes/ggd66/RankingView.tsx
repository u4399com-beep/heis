'use client'
// ============================================================
// clone-ggd66 RankingView — 1:1 精仿 ggd66.com 排行榜页
...
//       / .container > .breadcrumb (首页 > 排行榜)
//       / .container > .content > .content-left (h2 排行榜 + .class tab 切换 + .bookbox × N
//       含 .num/.bookinfo/.bookname/.author/.cat/.update) + .content-right #fengyou
//       (.search + h2 阅读排行榜 ul li) + .pages 分页 / .footer
    ...
    <>
      ...
      <div className="container">
        <ul className="breadcrumb">
          ...
          <li className="active">排行榜</li>
        </ul>

        <div className="content">
          <div className="content-left" id="rank-list">
            <h2>{TABS.find(t => t.id === tab)?.name || '排行榜'}</h2>
                ...
                {TABS.map(t => (
                  <li key={t.id}>
                      <a
                        ...
                        style={tab === t.id ? { color: '#fff', background: '#56ccb5', display: 'inline-block', padding: '2px 8px' } : undefined}
                      >{t.name}</a>
                  </li>
                ))}
              </ul>
            ...
            {loading ? (
              ...
              <div style={{ padding: 40, textAlign: 'center' }}>暂无排行数据</div>
            ) : (
              books.map((b, i) => (
