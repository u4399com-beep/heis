'use client'
// ============================================================
// clone-ggd66 SearchView — 1:1 精仿 ggd66.com 搜索结果页
// 参考: agent-ctx/probe-html2/probe-ggd66.html 中的 .content-right .search form +
//       .content-left 列表 + CSS .class / .bookbox .num / .pages
// 复刻: .header > .container (.header-left .logo + .header-right + .header-nav)
//       / .container > .breadcrumb (首页 > 搜索)
//       / .container > .content > .content-left (h2 "搜索: 关键词" + .search 表单 +
    ...
    <>
      ...
      <div className="container">
        <ul className="breadcrumb">
          ...
          <li className="active">搜索：{q || ''}</li>
        </ul>

        <div className="content">
          <div className="content-left" id="search-list">
            <h2>搜索：{q || ''}</h2>

            {/* 搜索表单 */}
            <div className="search hidden-xs" style={{ marginBottom: 10 }}>
              <form name="articlesearch" method="post" action="/search/" onSubmit={goSearch}>
                <input name="searchkey" type="text" className="text" id="searchkey" size={10} maxLength={50} placeholder="搜索从这里开始..." defaultValue={q || ''} autoComplete="off" />
