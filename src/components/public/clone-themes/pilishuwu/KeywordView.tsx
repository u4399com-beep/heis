'use client'
// ============================================================
// clone-pilishuwu KeywordView — 标签关键词落地页
// 参考: probe-pilishuwu-book.html 的 ret-search-list 结构 (与 CategoryList/SearchView 同模式)
// 复刻: .mod-top-wr header (搜索框预填 tag) / .mod-top-nav-wr nav /
//       .ui-wm .ret-main-wr .ret-main (.ret-search-head.clearfix
//       h1 "标签: X" + .ret-result-num em "N 个结果") + .ret-search-result
//       ul.ret-search-list li.ret-search-item (同 SearchView 结构) +
//       底部相关标签 ul.tags-show a.works-intro-tags-item (从 books 拆分 keywords)
...
export function KeywordView({ tag, books, loading, initialCategories }: KeywordViewProps) {
  ...
  // 从 books.category 拆分相关分类 (前 16 个不重复, BookItem 无 keywords 字段)
  const tagArr: string[] = Array.from(new Set(
              ...
              <form action="/module/search/search.php" method="get" onSubmit={goSearch}>
                ...
                <div id="top-search" className="mod-top-search">
                  <div className="mod-search-input-wr ui-left">
                    <input className="mod-search-input" type="text" name="key" placeholder="可搜索小说名/作者名" defaultValue={tag || ''} autoComplete="off" />
                  </div>
          ...
          <div className="ret-main">
            ...
            <div className="ret-search-head clearfix">
              <h1 className="ui-left" style={{ fontSize: 22, fontWeight: 'bold', color: '#333', padding: '8px 0', marginRight: 20 }}>
                标签: <span style={{ color: '#fd8929' }}>{tag || ''}</span>
              </h1>
            ...
            <div className="ret-search-result">
              {loading ? (
                ...
                <div style={{ padding: 40, textAlign: 'center', color: '#999' }}>
                  暂无 "<strong style={{ color: '#fd8929' }}>{tag}</strong>" 相关的书籍
                  <br />
                  <a
            ...
            {/* 底部相关标签 (works-intro-tags-item) */}
            {tagArr.length > 0 && (
              <div className="works-intro-tags" style={{ marginTop: 20, padding: 16, background: '#fff', borderTop: '1px solid #eee' }}>
                <p className="works-intro-tags">
                  <span className="ui-left" style={{ fontWeight: 'bold', marginRight: 8 }}>相关标签：</span>
                  <span className="tags-show">
                    {tagArr.map(t => (
                      <a
                        key={t}
                        href={`/search/3/${encodeURIComponent(t)}/1.html`}
                        className="works-intro-tags-item"
                        onClick={(e) => { e.preventDefault(); navigate({ view: 'keyword', tag: t }) }}
                      >{t}</a>
                    ))}
                  </span>
                </p>
