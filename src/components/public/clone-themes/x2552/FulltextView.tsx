'use client'
// ============================================================
// clone-x2552 FulltextView — 全本完本小说页
// 注: x2552 没有子页 probe, 按 JieQi CMS 标准完本列表页模板构造
//     (源站 URL /fulltxt/{page}.html, 列表风格与分类页一致)
// 复刻: .main.m_head (header + searchbox) / .main.m_menu (10 分类 + 全本, 全本 active) /
//       .main > .block (.blocktitle "全本小说列表" + .blockcontent 含 table 列表
//       thead 类别/书名/最新章节/作者/字数/更新 + tbody tr × N +
    ...
    <>
      ...
      <div className="main m_menu">
        <ul>
          ...
          <li className="current"><a href="/fulltxt/1_1.html" onClick={goFulltext}>全本</a></li>
      ...
      <div className="main">
        <div className="block">
          <div className="blocktitle"><i />{site.name}全本小说列表</div>
                ...
                <tbody>
                  {books.map(b => (
                    <tr key={b.id}>
                      ...
                      <td>
                        <a href={`/html/${b.categoryId || '0'}/${b.id}/${b.id}.html`} target="_blank" {...bookNavProps(navigate, b.id)}>{b.latestChapter || '已完结'}</a>
                      </td>
                      <td>{b.author}</td>
                      <td><span className="hottext">{formatWords(b.wordCount)}</span></td>
                      <td><span className="hottext">{statusLabel(b.status)}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
