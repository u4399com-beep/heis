'use client'
// ============================================================
// clone-x2552 KeywordView — 标签关键词落地页
// 注: x2552 没有子页 probe, 按 JieQi CMS 标准标签/关键词结果页模板构造
//     (源站搜索表单 action /modules/article/search.php 同样支持 tag 模式, 列表风格与分类页一致)
// 复刻: .main.m_head (header + searchbox 预填 tag) / .main.m_menu (10 分类 + 全本) /
//       .main > .block (.blocktitle "标签: tag 的相关小说列表" + .blockcontent 含
//       table 列表 thead 类别/书名/最新章节/作者/字数/更新 + tbody tr × N +
//       底部相关标签 .tags 行: 从 books.category 拆分前 16 个不重复) /
...
export function KeywordView({ tag, books, loading, initialCategories }: KeywordViewProps) {
  ...
  // 从 books.category 拆分相关分类 (前 16 个不重复, 与 tag 不同)
          ...
          <div>
            <form action="/modules/article/search.php" method="post" name="articlesearch" id="articlesearch" onSubmit={goSearch}>
              ...
              <dl className="fl searchbox">
                <dt>
                  <i />
                  <input type="text" name="searchkey" placeholder="搜索书名" defaultValue={tag || ''} autoComplete="off" />
                </dt>
                <dd>
      ...
      <div className="main">
        <div className="block">
          <div className="blocktitle">
            <i />
            标签 "<span className="hottext">{tag || ''}</span>" 的相关小说列表 (共 <span className="hottext">{books.length}</span> 本)
          </div>
          <div className="blockcontent">
            {loading ? (
              ...
              <div style={{ padding: 40, textAlign: 'center', color: '#999' }}>
                暂无 "<strong className="hottext">{tag}</strong>" 相关的书籍
                <br />
            ...
            {/* 底部相关标签 (.tags) */}
            {tagArr.length > 0 && (
              <p className="tags" style={{ marginTop: 16, padding: '8px 12px', borderTop: '1px dotted #E4E4E4' }}>
                <strong>相关标签：</strong>
                {tagArr.map((t, i) => (
                  <span key={t}>
                    ...
                    <a
                      ...
                      style={{ color: '#2f468f', textDecoration: 'none' }}
                    >{t}</a>
                  </span>
                ))}
              </p>
