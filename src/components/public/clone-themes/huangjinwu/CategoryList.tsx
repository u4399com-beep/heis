'use client'
// ============================================================
// clone-huangjinwu CategoryList — 1:1 精仿 huangjinwu.org 分类列表页
// 参考: huangjinwu.css 中 .breadcrumb/.filter-bar/.filter-tags/.filter-tag/
//       .book-list-item/.book-list-cover/.book-list-info/.book-list-title/
//       .book-list-desc/.book-list-meta/.book-badge/.pagination/.pagination-list/
//       .page-link/.page-info 等 selector
// (huangjinwu 无 category probe, 从首页 DOM + CSS class 推断子页结构)
// 复刻: .header-group (玻璃 header) + .main-content > .container
//       > .breadcrumb + .page-title (分类标题) + .filter-bar (.filter-tags
//       分类切换) + .book-list (book-list-item × N: .book-list-cover + .book-list-info
//       .book-list-title + .book-list-desc + .book-list-meta) + .pagination
//       .pagination-list .page-link × N + .footers
    ...
    return (
      <div className="pagination">
        <ul className="pagination-list">
          <li>
            <button type="button" className="page-link" disabled={page <= 1} onClick={() => onPage(page - 1)}>上一页</button>
          </li>
          {nums.map(n => (
            <li key={n}>
              <button type="button" className={`page-link${n === page ? ' active' : ''}`} onClick={() => onPage(n)} disabled={n === page}>{n}</button>
            </li>
          ))}
          <li>
            <button type="button" className="page-link" disabled={page >= totalPages} onClick={() => onPage(page + 1)}>下一页</button>
          </li>
        </ul>
      ...
      <div className="main-content">
        <div className="container">
          ...
          <h2 className="page-title">
            <span className="iconfont icon-sort"></span> {label}
          </h2>
          ...
          <div className="filter-bar">
            <div className="filter-tags">
              {navCats.map(c => (
                <a key={c.id} className={`filter-tag${label === c.name ? ' active' : ''}`} href={`/list/${c.id}`} onClick={(e) => goCat(e, c.id)}>
                  {c.name}
                </a>
              ))}
            </div>
          ...
          {/* 书籍列表 (book-list-item 横排: 封面 + 信息) */}
          <div className="book-list">
            {loading ? (
              <div className="empty-state"><p>加载中...</p></div>
            ) : !books.length ? (
              <div className="empty-state"><p>暂无该分类的书籍</p></div>
            ) : books.map(b => (
