'use client'
// ============================================================
// clone-shipsay CategoryList — 1:1 精仿 demo.shipsay.com 分类列表页
// 参考: agent-ctx/probe-html2/probe-shipsay.html (源站首页真实 DOM) +
//       public/clone-css/shipsay.css 的 .store / .store_left / #store_right /
//       .side_commend .flex li / .img_span / .w100 / .li_bottom / .pages
// 复刻: header>.container.head / .navigation>nav / .container>.store
//       .store_left>.side_commend.side_commend_width (p.title + ul.flex
//       li × N 含 .img_span a BookCover + span 类别/状态 + .w100 a h2 书名 +
//       p.indent 简介 + .li_bottom a 作者 + div em.orange 字数 + em.blue 日期) +
//       .pages 分页 + #store_right (ul li.store_title 全部分类 + ul li.store_title
//       热门小说 + 热门作者)
        ...
        <div className="store">
          ...
          <div className="store_left">
            <div className="side_commend side_commend_width">
              <p className="title"><i className="fa fa-book fa-lg">&nbsp;</i>{label}</p>
              {loading ? (
                <div style={{ padding: 40, textAlign: 'center', color: '#969ba3' }}>加载中...</div>
              ) : !books.length ? (
                <div style={{ padding: 40, textAlign: 'center', color: '#969ba3' }}>暂无书籍</div>
              ) : (
                <ul className="flex">
                  {books.map(b => (
                    <li key={b.id}>
          ...
          <div id="store_right">
            ...
            <ul>
              <li className="store_title">热门小说</li>
              {hotBooks.map(b => (
