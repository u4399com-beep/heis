'use client'
// ============================================================
// clone-aijjxs KeywordView — 1:1 精仿 aijjxs.com 关键词/标签落地页
// 参考: agent-ctx/probe-html2/probe-aijjxs-category.html 的 .cenMain + .catalog listbg
// 复刻: .top-float / .wrap > .top (logo+搜索) / .layout > .cenMain
//       (.articleInfo h1 关键词 + .body.filters 排序切换
//       + .catalog listbg × N) + aside (.panel.rank 热门标签下载
//       / .panel 热门作者 / .panel 相关标签)
...
export function KeywordView({ tag, books, loading, initialCategories }: KeywordViewProps) {
  ...
  // 相关标签: 取所有书的 keywords 字段拆分去重(如无则用书名首字)
      ...
      <div className="wrap">
        ...
        <main className="layout">
          <div className="cenMain">
            <div className="articleInfo"><h1>“{tag}”相关小说</h1></div>
          ...
          <aside>
            ...
            {relatedTags.length > 0 && (
              <article className="panel" style={{ marginTop: 14 }}>
                ...
                <div className="body tags">
                  {relatedTags.map(t => (
                    <a key={t} href="javascript:;" title={t} onClick={(e) => { e.preventDefault(); navigate({ view: 'keyword', tag: t }) }}>{t}</a>
                  ))}
                </div>
              </article>
            )}
