...
func selfTest() bool {
	...
	// ④ HTML→文本(含 &amp; 最后解码)
	conv := htmlToText("你好<br />  世界<b>x</b><br/><br/><br/> tail&nbsp;end")
	if conv != "你好\n世界x\n\ntail end" {
...
// htmlToText — ajax2 content HTML 片段 → 纯文本。
...
func fetchContent(chapterURL string) contentResult {
	...
	if !ok {
		return contentResult{OK: false, Error: fmt.Sprintf("u 必须为 deqixs 章节页 URL(/books/{aid}/{cid}.html), 收到: %s", truncStr(chapterURL, 120))}
	}
	...
	if !js1.OK {
		return contentResult{OK: false, AID: aid, CID: cid,
			Error: fmt.Sprintf("chapter.js.php 上游失败(%d%s)", js1.Status, ifStr(js1.Error != "", " "+js1.Error, ""))}
	}
	...
	if token == "" || timestamp == "" || nonce == "" {
		return contentResult{OK: false, AID: aid, CID: cid,
			Error: fmt.Sprintf("三参数提取失败(token=%s/ts=%s/nonce=%s), 响应头120B: %s",
				ifStr(token == "", "空", "OK"), ifStr(timestamp == "", "空", "OK"), ifStr(nonce == "", "空", "OK"),
				truncStr(jsText, 120))}
	}
	...
	if !aj.OK {
		return contentResult{OK: false, AID: aid, CID: cid,
			Error: fmt.Sprintf("ajax2.php 上游失败(%d%s)", aj.Status, ifStr(aj.Error != "", " "+aj.Error, ""))}
	}
	// GBK → UTF-8 解码
	...
	if err := json.Unmarshal(utf8Body, &j); err != nil {
		return contentResult{OK: false, AID: aid, CID: cid,
			Error: fmt.Sprintf("ajax2.php 响应非JSON(%dB): %s", len(utf8Body), truncStr(string(utf8Body), 80))}
	}
	if j.Status != 1 || j.Data.Content == "" {
		return contentResult{OK: false, AID: aid, CID: cid,
			Error: fmt.Sprintf("ajax2.php 业务失败(status=%d, message=%s)", j.Status, j.Message)}
	}
	return contentResult{OK: true, AID: aid, CID: cid, Content: htmlToText(j.Data.Content)}
}
...
func healthCheck() (map[string]any, error) {
	...
	return map[string]any{
		"upstreamReachable": upstreamReach,
