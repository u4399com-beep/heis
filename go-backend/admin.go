...
func adminDownloadsCreate(w http.ResponseWriter, r *http.Request) {
	...
	go func(jid, bid, bname string) {
		...
		chapters := []chRow{}
		// R73-D BUG-103 (P1, R72 交接 #4): 原查询无 LIMIT, 万章书 (10000+ 章)
		//   × ~5KB content/章 = 50MB+ 字符串 + chapters slice + strings.Builder
		//   峰值内存 ~100MB/下载任务. maxConcurrentDownloadJobs=3 → 峰值 300MB;
		//   downloadFilesMaxEntries=50 缓存 → 峰值 2.5GB → OOM 风险. 修复: 加
		//   LIMIT 1000 (与 adminBackupHandler backupBigBooksThreshold=200 同款
		//   "保守上限" 思路). 1000 章够用 (万章书用户可分批下载, 与 R72 交接 #4
		//   "下载任务一般不需要全章节" 一致). ORDER BY idx ASC 保证取前 1000 章
		//   (从头开始, 用户最需要的部分).
...
func adminBackupClearHandler(w http.ResponseWriter, r *http.Request) {
	...
	// R73-D BUG-105 (P3): 原 6 个 DELETE 不在事务内, 若中途某步失败 (e.g. Chapter
	//   DELETE 成功但 Book DELETE 失败) → 半清空脏状态 (Book 残留但 Chapter 全删,
	//   前台显示"0 章"的孤儿 Book 行). 修复: 包裹事务, 任一步失败 Rollback 还原全
	//   部 DELETE (与 adminSettingsUpdate / adminBackupRestoreHandler 同款事务方法论).
	//   注: DELETE 顺序按依赖反序 (先删依赖方 TaskLog/Task/DownloadJob/BookTag/
	//   Chapter, 后删被依赖方 Book), 即使无外键约束也合理.
