...
func ApplyTransform(value string, rule FieldRule) string {
        ...
        switch rule.Decode {
        case "base64-json":
                if m := regexp.MustCompile(`base64,([A-Za-z0-9+/=_-]+)`).FindStringSubmatch(v); m != nil {
...
func cssSelect(doc *goquery.Document, scope *goquery.Selection, expr string) *goquery.Selection {
        ...
        fixed := regexp.MustCompile(`#(\d[\w-]*)`).ReplaceAllString(expr, `[id="$1"]`)
        if fixed != expr {
                return run(fixed)
        }
        return nil
        ...
        for _, p := range parts {
                ...
                if strings.HasPrefix(p, "[") && strings.HasSuffix(p, "]") {
                        ...
                        if strings.HasPrefix(inner, "?(") && strings.HasSuffix(inner, ")") {
                                ...
                                if m := regexp.MustCompile(`@?\.?(\w+)\s*==\s*"?(.*?)"?\s*$`).FindStringSubmatch(expr); m != nil {
                                        tokens = append(tokens, jsonToken{
                                                kind: tokenFilter,
                                                fk:   m[1],
                                                fv:   m[2],
